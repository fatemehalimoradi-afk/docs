...
Knit bundles on Tapcart use the **same active bundles** as your Shopify store. Add to cart goes to **Tapcart's native cart**. The Knit block attaches the discount info — no extra merchant step. The discount still applies in Shopify checkout. Keep the Knit automatic discount **Active** in Shopify admin → **Discounts**.
...
Then complete a test order. Each bundle line in Shopify admin → **Orders** should show `__FB_ATC_UID` and the discount. If items are in the cart with no discount, see [Troubleshooting](/troubleshooting).
